...
> **instruct** models (clean JSON) and the picture flipped (see table). Auditing your own
> harness is part of honest measurement.
>
> ⚠️ Free-model availability is flaky: of 5 models tried, 3 (`gemma-4-31b`, `qwen3-coder`,
> `llama-3.2-3b`) returned solid 429s at test time — no data for them. The ~10–12 s latency is
> also a property of free endpoints; a paid model would be faster.
...
4. **The LLM wins exactly where rules/ML are weak — classifying new vocabulary.** Category:
   nex-n2-pro **97%**, nemotron **83%** vs **48%** for rules and **59%** for ML. Words the
   author never anticipated ("rapeseed", "superphosphate", "expanded clay") the LLM recognizes,
   the rules don't. nex also hit **100% routing**. The price: **~10–12 s** per query (free
   endpoints) and, for nemotron, one stray spec on a non-big-bag query (1/15) — minor off-topic;
   nex 0%.
