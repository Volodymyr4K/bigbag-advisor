...
worth it**. Anti-hype here isn't a slogan but a measured result: on our data neither
classical ML nor a (free) LLM beat the rules — details and caveats below.
...
LLM model: **`openai/gpt-oss-120b:free`** (via OpenRouter). \* `$0` because it's a free model,
so the cost column proves nothing here; a paid model would show a real price (the bench
computes it — see below). Separate "invented a spec for a non-big-bag query" metric:
...
4. **The LLM lost to both simple baselines here — measured, not assumed.** I openly wrote the
   opposite hypothesis ("zero-shot LLM will lift category above 48–59%"). The real numbers
   **refuted it**:
   - routing: grounded **57%**, no-ground **64%** — both **worse** than rules (98%) and ML
     (84%). The model often failed to read colloquial "беги"/"тара" as a big-bag and routed
     to "general";
   - category: **48%** — exactly rules, **worse** than ML (59%). No gain at all;
   - speed: **~19–23 s** per query vs **0 ms** for rules/ML;
   - hallucination showed up not in sizes (0% out-of-catalog) but in the no-ground LLM
     **answering 2 of 15 non-big-bag queries with a spec** (13%) where it should have stayed
     silent and routed to a department. **Grounded: 0%** — that's where the knowledge base
     genuinely helped (kept it on-topic), even if it didn't lift accuracy.

**Honest takeaway from this measurement:** for this structured task the expensive LLM **isn't
worth it** — simple rules are faster, more accurate at routing, and free. Anti-hype here isn't
a stance, it's the scoreboard.

**Important caveat (so as not to overclaim the other way):** this is **one** model — the free
reasoning `gpt-oss-120b:free` (the only one that got past the key's $0 limit). A stronger paid
model (e.g. `gpt-4o-mini`) could well narrow the gap — that's the **next measurement**, not
grounds to declare "LLMs don't work" in general. The bench is ready: just change `LLM_MODEL`.

```bash
cp .env.example .env          # add OPENROUTER_API_KEY (+ LLM_MODEL if needed)
npm run bench                 # re-measures rules / ML / LLM / LLM-no-ground on held-out
```
...
- The LLM was measured on **one** free model (`gpt-oss-120b:free`) — the only one that got
  past the key's $0 limit. A stronger paid model may give different numbers; this is the next
  measurement, not a final verdict on LLMs. The ~20 s latency is also specific to this model.
